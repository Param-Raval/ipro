...
def main(parser):
    ...
    if parser.load is not None:
        ...
        G1_weights = torch.load('./app/checkpoints/ST-CGAN_G1_'+parser.load+'.pth', map_location=torch.device('cpu'))
        G1.load_state_dict(fix_model_state_dict(G1_weights))

        G2_weights = torch.load('./app/checkpoints/ST-CGAN_G2_'+parser.load+'.pth', map_location=torch.device('cpu'))
